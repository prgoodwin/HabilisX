...
namespace MicrosoftResearch.Infer.Distributions
{
		...
		where Distribution : SettableTo<Distribution>
	{
		...
		public void SetTo(ArrayEstimator<ItemEstimator, DistributionArray, Distribution, Sample> value)
		{
			if (value.estimators.Length != estimators.Length) throw new ArgumentException("value.estimators.Length ("+value.estimators.Length+") != estimators.Length ("+estimators.Length+")");
			for (int i = 0; i < estimators.Length; i++) {
	...
	public static class ArrayEstimator<T>
	{
		...
		public static TEstArray Array<TEstArray, TEstimator>(TEstimator[][][] array, Type[] estArrTypes)
			where TEstimator : Estimator<T>
		{
			MethodInfo method = new Func<TEstimator[][][], Type, TEstArray>(ArrayEstimator<T>.Array111<TEstArray, TEstimator, object, object>).Method.GetGenericMethodDefinition();
			...
			where TEstimator : Estimator<T>
		{
			return (TEstArray)Activator.CreateInstance(arrayType, array.Length, new Converter<int, TMiddle>(
					delegate(int i)
					{
						return (TMiddle)Activator.CreateInstance(typeof(TMiddle), array[i].Length, new Converter<int, TInner>(
							delegate(int j)
							{
