...
namespace MicrosoftResearch.Infer.Distributions
{
	...
	public class SampleList<T> : Accumulator<T>
	{
		...
		/// Samples
		/// </summary>
		public IList<T> Samples { get { return samples.AsReadOnly(); } }
		...
		where TDist : ICloneable
	{
		...
		/// Samples
		/// </summary>
		public IList<TDist> Conditionals { get { return conditionals.AsReadOnly(); } }
		...
		where TDist : IDistribution<T>, Sampleable<T>
	{
		...
		public GibbsMarginal(TDist distPrototype, int burnIn, int thin, bool estimateMarginal, bool collectSamples, bool collectDistributions)
		{
			...
			if (estimateMarginal) {
				...
				try { this.estimator = ArrayEstimator.CreateEstimator<TDist, T>(distPrototype, true); } catch (Exception) { }
		...
		/// </summary>
		public Estimator<TDist> Estimator { get { return estimator; } }
