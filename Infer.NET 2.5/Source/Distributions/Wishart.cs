...
namespace MicrosoftResearch.Infer.Distributions
{
		...
		SettableToWeightedSum<Wishart>, CanGetAverageLog<Wishart>, CanGetLogNormalizer
	{
		...
		public void SetToRatio(Wishart numerator, Wishart denominator, bool forceProper = false)
		{
			if (numerator.IsPointMass) {
				...
				shape = numerator.shape + (0.5 * (Dimension + 1) - denominator.shape);
				rate.SetToDifference(numerator.rate, denominator.rate);
				//if (forceProper) throw new NotImplementedException();
	...
	where T : CanGetMeanAndVariance<PositiveDefiniteMatrix, PositiveDefiniteMatrix>, CanSetMeanAndVariance<PositiveDefiniteMatrix, PositiveDefiniteMatrix>, SettableToUniform, SettableTo<T>
		...
		{
			if (result == null) result = new Wishart(mean.Rows);
			...
			if (delta <= 2e-16) return Wishart.PointMass(mean);
