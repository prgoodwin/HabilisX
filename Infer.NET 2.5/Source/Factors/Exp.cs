...
namespace MicrosoftResearch.Infer.Factors
{
	...
	public class ExpOp
	{
		...
		public static Gaussian DAverageLogarithm([Proper] Gamma exp, [Proper] Gaussian d, Gaussian to_d)
		{
			...
			if (result.IsPointMass) throw new Exception();
			return result;
	...
	public static class ExpOp_Laplace
	{
		...
		{
			if (exp.IsPointMass) return ExpOp.DAverageConditional(exp.Point);
			Gaussian dPost = d*to_d;
	...
	internal static class ExpOp_Laplace3_Slow
	{
		...
		const double inc = (xmax-xmin)/(n-1);
		static Matrix dd = null;

		static DenseVector Aux([SkipIfUniform] Gamma exp, [Proper] Gaussian d, DenseVector logh, DenseVector aux)
		{
			double b = exp.Rate;
			...
			for (int i = 0; i < n; i++) {
				...
				if (target[i] < 0) throw new Exception();
