...
{
	/// <summary>
	/// Provides outgoing messages for <see cref="Factor.Rotate"/>, given random arguments to the function.
	/// </summary>
	...
	public static class RotateOp
	{
		...
		public static Gaussian XAverageLogarithm([SkipIfUniform] VectorGaussian rotate, [Proper] WrappedGaussian angle)
		{
			...
			if (rotate.Precision[1, 1] != prec) throw new ArgumentException("rotate.Precision is not spherical");
#if false
...
#endif
			if (double.IsNaN(mean)) throw new ApplicationException("result is nan");
